  ...
  if (routeId) {
    return (
      <div className={`min-h-screen ${bgMain} ${textMain} transition-colors duration-300`}>
        {isDark && <div className="fixed inset-0 opacity-5 pointer-events-none"><div className="matrix-rain"></div></div>}
        ...
        {toast.show && (
          <div className="fixed top-4 right-4 z-50 animate-slide-in">
            <div className={`px-6 py-4 rounded-xl shadow-2xl backdrop-blur-xl border ${toast.type === 'success' ? 'bg-emerald-500/90 border-emerald-400 text-white' : 'bg-red-500/90 border-red-400 text-white'}`}>
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 md:py-6">
            <div className="flex items-center justify-between">
              <div className="flex items-center space-x-3">
                ...
                <div>
                  <h1 className={`text-xl md:text-3xl font-black bg-gradient-to-r ${accentColor} bg-clip-text text-transparent tracking-tight`}>CodeSpace</h1>
                  <p className={`${textSecondary} text-xs md:text-sm mt-0.5 font-mono`}>/{routeId}</p>
                </div>
              </div>
              <div className="flex items-center gap-2">
                <button onClick={toggleTheme} className={`p-2 md:p-2.5 ${isDark ? 'bg-slate-800 hover:bg-slate-700' : 'bg-gray-200 hover:bg-gray-300'} rounded-lg transition-all`}>
                  {isDark ? <Sun className="w-4 h-4 text-yellow-400" /> : <Moon className="w-4 h-4 text-slate-700" />}
                </button>
                <a href="/" className={`px-4 py-2 ${isDark ? 'bg-slate-800 hover:bg-slate-700' : 'bg-gray-200 hover:bg-gray-300'} rounded-lg text-sm font-semibold transition-all flex items-center gap-2`}>
          ...
          {loading ? (
            <div className="flex items-center justify-center py-20">
              <div className="text-center">
                <div className={`inline-flex items-center justify-center w-16 h-16 ${isDark ? 'bg-slate-800' : 'bg-gray-200'} rounded-2xl mb-4`}>
                  <div className="animate-spin"><Terminal className="w-8 h-8 text-cyan-500" /></div>
                </div>
            ...
            <div className={`${bgCard} backdrop-blur-xl rounded-2xl shadow-2xl border ${borderColor} p-10 md:p-16 text-center`}>
              ...
              <p className={`${textSecondary} text-base md:text-lg mb-8`}>
                The route <span className="text-cyan-500 font-mono font-bold">/{routeId}</span> doesn't exist.
              </p>
              <a href="/" className={`inline-flex items-center gap-2 px-8 py-3 bg-gradient-to-r ${accentColor} text-white rounded-xl font-bold hover:shadow-lg transition-all`}>
            ...
            <div className={`${bgCard} backdrop-blur-xl rounded-2xl shadow-2xl border ${borderColor} p-6 md:p-8`}>
              <div className="flex items-center justify-between mb-6 flex-wrap gap-4">
                ...
                <div className="flex items-center gap-2">
                  {!isEditing ? (
                    <>
                      <button onClick={startEditing} className={`px-4 py-2 text-sm font-semibold flex items-center gap-2 transition-all ${isDark ? 'bg-slate-800 hover:bg-slate-700 text-blue-400' : 'bg-gray-200 hover:bg-gray-300 text-blue-600'} rounded-xl`}>
                        <Edit3 className="w-4 h-4" /><span className="hidden sm:inline">Edit</span>
                      </button>
                      <button onClick={() => { navigator.clipboard.writeText(fetchedCode); showToast('Copied to clipboard!', 'success'); }} className={`px-4 py-2 text-sm font-semibold flex items-center gap-2 transition-all ${isDark ? 'bg-slate-800 hover:bg-slate-700 text-cyan-400' : 'bg-gray-200 hover:bg-gray-300 text-cyan-600'} rounded-xl`}>
                        <Copy className="w-4 h-4" /><span className="hidden sm:inline">Copy</span>
                      </button>
                    </>
                  ) : (
                    <>
                      <button onClick={handleUpdateCode} disabled={updating} className={`px-4 py-2 text-sm font-semibold flex items-center gap-2 transition-all bg-gradient-to-r ${accentColor} text-white rounded-xl disabled:opacity-50`}>
                        <Check className="w-4 h-4" /><span className="hidden sm:inline">{updating ? 'Saving...' : 'Save'}</span>
                      </button>
                      <button onClick={cancelEditing} disabled={updating} className={`px-4 py-2 text-sm font-semibold flex items-center gap-2 transition-all ${isDark ? 'bg-slate-800 hover:bg-slate-700 text-red-400' : 'bg-gray-200 hover:bg-gray-300 text-red-600'} rounded-xl disabled:opacity-50`}>
              ...
              <div className={`relative ${isDark ? 'bg-black/40 border-slate-700' : 'bg-gray-900 border-gray-700'} border rounded-xl overflow-hidden`}>
                <div className={`flex items-center justify-between px-4 py-2 ${isDark ? 'bg-slate-800/50' : 'bg-gray-800/90'} border-b ${isDark ? 'border-slate-700' : 'border-gray-700'}`}>
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-xs text-gray-400 font-mono ml-3">{routeId}.txt</span>
                  </div>
                  <div className="flex items-center gap-2">
              ...
              {isEditing && <p className={`text-xs ${textSecondary} mt-3 flex items-center gap-2`}><Zap className="w-3 h-3" /><span>Make your changes and click Save to update the code</span></p>}
            </div>
          )}
        </div>
        ...
        <style>{`
          ...
          .matrix-rain { background: linear-gradient(transparent 0%, rgba(6, 182, 212, 0.05) 50%, transparent 100%); background-size: 100% 200%; animation: rain 3s linear infinite; }
      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 md:py-10">
        <div className={`${bgCard} backdrop-blur-xl rounded-2xl shadow-2xl border ${borderColor} p-6 md:p-8`}>
          <div className="flex items-center gap-3 mb-6">
            ...
            <div>
              <h2 className={`text-xl md:text-2xl font-black ${textMain}`}>Save Your Code</h2>
          ...
          <div className="space-y-5">
            <div>
              ...
              <input type="text" value={id} onChange={(e) => setId(e.target.value)} placeholder="Enter unique identifier → my-awesome-function" className={`w-full px-4 md:px-5 py-3 md:py-4 ${bgInput} border ${borderColor} rounded-xl ${textMain} placeholder-gray-500 focus:ring-2 focus:ring-cyan-500 focus:border-transparent outline-none transition-all text-sm md:text-base font-mono`} />
            ...
            <div>
              ...
              <div className={`${isDark ? 'bg-black/40 border-slate-700' : 'bg-gray-900 border-gray-700'} border rounded-xl overflow-hidden`}>
                ...
                <div className="flex">
                  ...
                  <textarea value={code} onChange={(e) => setCode(e.target.value)} placeholder="// Start typing your code here...
const hackerMode = true;

function deployCode() {
  return 'Ready to ship! 🚀';
}" rows="12" className={`flex-1 px-4 py-4 bg-transparent text-gray-200 placeholder-gray-600 outline-none font-mono text-xs md:text-sm leading-6 resize-none`} />
                </div>
              </div>
              
              <p className={`text-xs ${textSecondary} mt-2 flex items-center gap-2`}>
                <Zap className="w-3 h-3" /><span>If ID exists .. Then you can edit the code by visiting <span className="text-cyan-500">/{id || 'your-identifier'}</span></span>
              </p>
            </div>

            <button onClick={handleSaveCode} disabled={loading} className={`w-full bg-gradient-to-r ${accentColor} text-white py-3 md:py-4 rounded-xl font-bold text-sm md:text-base hover:shadow-xl transition-all disabled:opacity-50 disabled:cursor-not-allowed ${isDark ? 'shadow-cyan-500/20 hover:shadow-cyan-500/40' : 'shadow-cyan-600/20 hover:shadow-cyan-600/40'} hover:scale-[1.02] active:scale-[0.98]`}>
              {loading ? (
                <span className="flex items-center justify-center gap-2">
                  <div className="animate-spin"><Terminal className="w-5 h-5" /></div>
                  Deploying...
                </span>
              ) : (
                <span className="flex items-center justify-center gap-2">
